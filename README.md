<div align="center">

# Muflih, Abdullah Mu'adz

Jr. Data Scientist — Applied Computer Vision / Edge AI

camera → detection → events → services → dashboard

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=16&duration=3500&pause=1200&color=58A6FF&center=true&vCenter=true&width=520&lines=real-time+video+analytics;edge+devices;still+learning+everyday;trying+to+understand+systems;one+step+at+a+time."/>

</div>

---

## Work

* fire & smoke detection
* restricted area detection
* people counting analytics
* face search & recognition
* license plate recognition
* annotated RTSP streaming

---

## Systems I Explore

streaming inference • event pipelines • background workers • edge-to-backend processing

---

## Stack

### Languages

![Python](https://img.shields.io/badge/python-333?style=flat\&logo=python)

### Backend & Processing

![FastAPI](https://img.shields.io/badge/fastapi-333?style=flat\&logo=fastapi)
![Celery](https://img.shields.io/badge/celery-333?style=flat)
![Kafka](https://img.shields.io/badge/apache_kafka-333?style=flat\&logo=apachekafka)
![RabbitMQ](https://img.shields.io/badge/rabbitmq-333?style=flat\&logo=rabbitmq)
![Redis](https://img.shields.io/badge/redis-333?style=flat\&logo=redis)

### Data & Storage

![PostgreSQL](https://img.shields.io/badge/postgresql-333?style=flat\&logo=postgresql)
![ClickHouse](https://img.shields.io/badge/clickhouse-333?style=flat\&logo=clickhouse)

### Apps & Visualization

![Streamlit](https://img.shields.io/badge/streamlit-333?style=flat\&logo=streamlit)

### Dev & Environment

![Docker](https://img.shields.io/badge/docker-333?style=flat\&logo=docker)
![Linux](https://img.shields.io/badge/linux-333?style=flat\&logo=linux)
![Fedora](https://img.shields.io/badge/fedora-333?style=flat\&logo=fedora)
![Ubuntu](https://img.shields.io/badge/ubuntu_server-333?style=flat\&logo=ubuntu)

---

## Home Lab

self-hosted services • local AI inference • ESP32 / ESP32-CAM experiments

---

<p align="center">
<img src="https://streak-stats.demolab.com?user=MuflihMA&theme=tokyonight&hide_border=true"/>
</p>
